...
# ===========================================
# PARA GRAFICAR RATE primer y segundo saque
# ===========================================



# Crear tabla con ambas métricas: 1er y 2do saque
resumen_saque_1y2 <- cerundolo_matches[, .(
  rate_pg_primer_saque = mean(rate_pg_primer_saque, na.rm = TRUE),
  rate_pg_segundo_saque = mean(rate_pg_segundo_saque, na.rm = TRUE)
), by = tourney_name]

# Mantener orden cronológico
resumen_saque_1y2$tourney_name <- factor(resumen_saque_1y2$tourney_name, levels = orden_torneos)
resumen_saque_1y2 <- resumen_saque_1y2[order(tourney_name)]

# Convertir a formato largo para graficar
resumen_long_saque <- melt(resumen_saque_1y2, 
                           id.vars = "tourney_name", 
                           variable.name = "tipo_rate", 
                           value.name = "valor")

setDT(resumen_long_saque)

# Renombrar tipo_rate para hacerlo más legible
resumen_long_saque[, tipo_rate := fcase(
  tipo_rate == "rate_pg_primer_saque", "Rate 1er Saque",
  tipo_rate == "rate_pg_segundo_saque", "Rate 2do Saque"
)]

# Gráfico con etiquetas
ggplot(resumen_long_saque, aes(x = tourney_name, y = valor, color = tipo_rate, group = tipo_rate)) +
  geom_line(size = 1.5) +
  geom_point(size = 3.5) +
  geom_text(aes(label = round(valor, 2)), 
            vjust = -1,
            ...
            nudge_y = 0.02,
            size = 4) +
  labs(title = "Rate 1er Saque vs 2do Saque - Juan Manuel Cerúndolo", 
       x = "", y = "") +
  theme_minimal() +
  theme(
...
# ============================
# RANKING 1ER Y 2DO SAQUE
# ============================

# Calcular promedios por torneo
ranking_saque <- cerundolo_matches[, .(
  rate_pg_primer_saque = mean(rate_pg_primer_saque, na.rm = TRUE),
  rate_pg_segundo_saque = mean(rate_pg_segundo_saque, na.rm = TRUE)
), by = tourney_name]

# Ordenar rankings
ranking_primer_saque <- ranking_saque[order(-rate_pg_primer_saque)]
ranking_segundo_saque <- ranking_saque[order(-rate_pg_segundo_saque)]
...
cat("========== TOP 5 – 1er Saque ==========\n")
print(head(ranking_primer_saque, 5))

cat("\n========== BOTTOM 5 – 1er Saque ==========\n")
print(tail(ranking_primer_saque, 5))

cat("\n========== TOP 5 – 2do Saque ==========\n")
print(head(ranking_segundo_saque, 5))

cat("\n========== BOTTOM 5 – 2do Saque ==========\n")
print(tail(ranking_segundo_saque, 5))


# ============================
# TABLA COMBINADA CON RANKINGS
# ============================

ranking_saque[, rank_1er := frank(-rate_pg_primer_saque, ties.method = "min")]
ranking_saque[, rank_2do := frank(-rate_pg_segundo_saque, ties.method = "min")]

# Ordenar por performance global (promedio de ambos ranks)
ranking_saque[, rank_global := (rank_1er + rank_2do) / 2]
ranking_global <- ranking_saque[order(rank_global)]

cat("\n========== RANKING GLOBAL (1er + 2do Saque) ==========\n")
