...
# Merge para agregar w_rank
dbm <- merge(dbm, rank_expanded[, .(date_rank, player, rank_position)], 
             by.x = c("date_match", "w_player"), 
             by.y = c("date_rank", "player"), 
             all.x = TRUE)

# Renombrar la columna rank_position a w_rank
setnames(dbm, "rank_position", "w_rank")

# Merge para agregar l_rank
dbm <- merge(dbm, rank_expanded[, .(date_rank, player, rank_position)], 
             by.x = c("date_match", "l_player"), 
             by.y = c("date_rank", "player"), 
             all.x = TRUE)
...
ganados <- dbm[l_rank < 11]
perdidos <- dbm[w_rank <11]
