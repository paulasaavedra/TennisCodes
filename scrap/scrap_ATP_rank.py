...
for actual_week in dates:
    ...
    url = (
        "https://www.atptour.com/en/rankings/singles?RankRange=1-5000&Region=all&DateWeek="
        + actual_week
    )
    ...
    for element in elements:
        # rank
        rank_element = (
            element.find("td", class_="rank bold heavy tiny-cell")
            .text.rstrip()
            .lstrip()
        )
        ...
        try:
            country_element = (
                element.find("svg", class_="flag").find("use")["href"][-3:].upper()
            )
        except:
        ...
        fila = [
            ...
            df.iloc[i]["age"],
            df.iloc[i]["points"],
            df.iloc[i]["points change"],
            df.iloc[i]["tourn played"],
            df.iloc[i]["dropping"],
            df.iloc[i]["next best"],
        ]
    ...
    # df.drop_duplicates()
    file_path = "/Users/paula/Documents/TennisData/TennisData/ATP_ranking/rank_weeks/"
    actual_week = actual_week.replace("-", "")
    df_all_data.to_csv(
        file_path + "ranking_" + actual_week + ".csv", header=False, index=False
    )
