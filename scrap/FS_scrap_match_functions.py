...
def scrap_match (URL):
    ...
    if status == 'Walkover' or status == 'WO' or status == 'W/O':
        ...
        if winner == 'home':
            ...
            elements = [tourney_level, tourney_name, country, surface, round_match, date_match, h_player, h_nac,
                        h_seed, a_player, a_nac, a_seed, h_list, h_1list, h_2list, h_3list, h_4list, h_5list,
                        a_list, a_1list, a_2list, a_3list, a_4list, a_5list, timestart, score, status, best, time_total,
                        time_1s, time_2s, time_3s, time_4s, time_5s, t_list, t_1list, t_2list, t_3list, t_4list, t_5list,
                        pointbypoint, anotations, winner]
        else:
            elements = [tourney_level, tourney_name, country, surface, round_match, date_match, a_player, a_nac,
                        a_seed, h_player, h_nac, h_seed, a_list, a_1list, a_2list, a_3list, a_4list, a_5list,
        ...
        if len(winner_listLastName) > 2:
            ...
            if posible_lastname[-1]=='.' and posible_lastname[0] == h_player[0] and winner_lastName == h_player.split(' ')[-1]:
                winner = 'home'
            elif posible_lastname[-1]!='.' and posible_lastname in h_player.split(' ') and winner_lastName in h_player.split(' '):
                winner = 'home'
            else:
                winner = 'away'
        else:
            ...
            if winner_lastName == h_player.split(' ')[-1] and inicial == h_player.split(' ')[0][0]:
                winner = 'home'
            else:
                winner = 'away'
        ...
        h_nac = soup.find(class_='smh__participantName smh__home').find('span')['title'] #Dato
        a_nac = soup.find(class_='smh__participantName smh__away').find('span')['title'] #Dato
        
        h_seed = soup.find(class_='smh__participantName smh__home').text.split('(')[-1][:-1]
        a_seed = soup.find(class_='smh__participantName smh__away').text.split('(')[-1][:-1]
        ...
        if winner == 'home':
            if h_5set != '' and a_5set != '':
                score = h_1set + '-' + a_1set + ' ' + h_2set + '-' + a_2set + ' ' + h_3set + '-' + a_3set + ' ' + h_4set + '-' + a_4set + ' ' +h_5set + '-' + a_5set
            elif h_4set != '' and a_4set != '':
                score = h_1set + '-' + a_1set + ' ' + h_2set + '-' + a_2set + ' ' + h_3set + '-' + a_3set + ' ' + h_4set + '-' + a_4set
            elif h_3set != '' and a_3set != '':
                score = h_1set + '-' + a_1set + ' ' + h_2set + '-' + a_2set + ' ' + h_3set + '-' + a_3set
            ...
            if h_5set != '' and a_5set != '':
                score = a_1set + '-' + h_1set + ' ' + a_2set + '-' + h_2set + ' ' + a_3set + '-' + h_3set + ' ' + a_4set + '-' + h_4set + ' ' + a_5set + '-' + h_5set
            elif h_4set != '' and a_4set != '':
                score = a_1set + '-' + h_1set + ' ' + a_2set + '-' + h_2set + ' ' + a_3set + '-' + h_3set + ' ' + a_4set + '-' + h_4set
            elif h_3set != '' and a_3set != '':
                score = a_1set + '-' + h_1set + ' ' + a_2set + '-' + h_2set + ' ' + a_3set + '-' + h_3set
            elif h_2set != '' and a_2set != '':
        ...
        pointbypoint = {}
        # order of service
        service_h = ['s-r','r-s','s-r','r-s','s-r','r-s',
                     's-r','r-s','s-r','r-s','s-r','r-s']
        service_a = ['r-s','s-r','r-s','s-r','r-s','s-r',
                     'r-s','s-r','r-s','s-r','r-s','s-r']
        # Order of service - tiebreak
        service_h_tb = ['s-r','r-s','r-s','s-r','s-r','r-s',
                        'r-s','s-r','s-r','r-s','r-s','s-r',
                        's-r','r-s','r-s','s-r','s-r','r-s',
                        'r-s','s-r','s-r','r-s','r-s','s-r',
                        's-r','r-s','r-s','s-r','s-r','r-s',
                        'r-s','s-r','s-r','r-s','r-s','s-r']
        service_a_tb = ['r-s','s-r','s-r','r-s','r-s','s-r',
                        's-r','r-s','r-s','s-r','s-r','r-s',
                        'r-s','s-r','s-r','r-s','r-s','s-r',
                        's-r','r-s','r-s','s-r','s-r','r-s',
                        'r-s','s-r','s-r','r-s','r-s','s-r',
        ...
        if tabs != None and ('Stats' in tabs.text or 'stats' in tabs.text):
            try:
                ...
                for stat in stats:
                    t_service = stat.find_all(class_='_simpleText_1loh2_4 _webTypeSimpleText01_1loh2_8 _bold_1loh2_57')                       
                ...
                if h_1set!='' and a_1set!='':
                    ...
                    for game in games:
                        if i<12:
                            ...
                            if len(service[0])==1:
                                pointbypoint['1set']['service'] = pointbypoint['1set']['service'] + service_h_tb [: len(games)-13]
                            else:
                ...
                if h_2set!='' and a_2set!='' and (h_2set!='0' or a_2set!='0'):
                    ...
                    for game in games:
                        if i<12:
                            ...
                            if len(service[0])==1:
                                pointbypoint['2set']['service'] = pointbypoint['2set']['service'] + service_h_tb [: len(games)-13]
                            else:
                                pointbypoint['2set']['service'] = pointbypoint['2set']['service'] + service_a_tb [: len(games)-13]
                            break
                ...
                if h_3set!='' and a_3set!='' and (h_3set!='0' or a_3set!='0'):
                    ...
                    for game in games:
                        if i<12:
                            ...
                            if len(service[0])==1:
                                pointbypoint['3set']['service'] = pointbypoint['3set']['service'] + service_h_tb [: len(games)-13]
                            else:
                                pointbypoint['3set']['service'] = pointbypoint['3set']['service'] + service_a_tb [: len(games)-13]
                            break
                ...
                if h_4set!='' and a_4set!='' and (h_4set!='0' or a_4set!='0'):
                    ...
                    for game in games:
                        if i<12:
                            ...
                            if len(service[0])==1:
                                pointbypoint['4set']['service'] = pointbypoint['4set']['service'] + service_h_tb [: len(games)-13]
                            else:
                                pointbypoint['4set']['service'] = pointbypoint['4set']['service'] + service_a_tb [: len(games)-13]
                            break
                ...
                if h_5set!='' and a_5set!='' and (h_5set!='0' or a_5set!='0'):
                    ...
                    for game in games:
                        if i<12:
                            ...
                            if len(service[0])==1:
                                pointbypoint['5set']['service'] = pointbypoint['5set']['service'] + service_h_tb [: len(games)-13]
                            else:
                                pointbypoint['5set']['service'] = pointbypoint['5set']['service'] + service_a_tb [: len(games)-13]
                            break
        ...
        if winner == 'home':
            ...
            elements = [tourney_level, tourney_name, country, surface, round_match, date_match, a_player, a_nac,
                        ...
                        time_1s, time_2s, time_3s, time_4s, time_5s,  t_list,  t_1list,  t_2list,  t_3list, t_4list,
                        t_5list, pointbypoint, anotations, winner]
    name_file = tourney_name + '_'  +str(date_match)[:-4] + '_' + h_player + '_' + a_player + '_' + datetime.now().strftime("%H:%M:%S").replace(':','') +'.csv' 
    data_simple = pd.DataFrame(elements[:12] + elements [24:34] + elements[-2:]).T
    d_simple_check = 'SI'
    data_simple.to_csv("C:/Users/Paula/Documents/Projects/TennisData/FS_matches/simple/" + name_file, index=False, header=None)
    ...
    if status.lower() == 'walkover':
        ...
        labels_concat_double = ['w_' + label for label in labels_concat] + ['l_' + label for label in labels_concat]
    ...
    d_stat_check = 'SI'
    df_data_stat.to_csv("C:/Users/Paula/Documents/Projects/TennisData/FS_matches/stats/" + name_file, index=False, header=True)
    print('Se guardo data_stat de ' + tourney_name + ' ' + h_player + '_' + a_player)
    # arranca pbyp
    if status.lower() == 'walkover':
        data_pbyp_wo = {'Columna1': ['WO'], 'Columna2': [None], 'Columna3': [None], 'Columna4': [None], 'Columna5': [None], 'Columna6': [None]}
        ...
        for valor in point_by_point.values():
            ...
            if elements[-1]=='home':
                ...
                df_servicio['r'] = df_servicio['r'].replace('away', elements[10])
                df_servicio['s'] = df_servicio['s'].replace('home', elements[6])
                df_servicio['s'] = df_servicio['s'].replace('away', elements[10])
            else:
                df_servicio = pd.DataFrame(service, columns=['Servicio'])
                # Dividir los elementos en las columnas "recibe" y "saca"
                df_servicio[['home', 'away']] = df_servicio['Servicio'].str.split('-', expand=True)
                ...
                # Reemplazar 'r' por "recibidor" en la columna "recibe"
                df_servicio['r'] = df_servicio['r'].replace('home', elements[10])
                df_servicio['r'] = df_servicio['r'].replace('away', elements[6])
                df_servicio['s'] = df_servicio['s'].replace('home', elements[10])
            ...
            df_aux = pd.DataFrame.from_dict(valor, orient='index', columns=['MatchScore'])
            ...
            df_games['cambios'] = (df_games['games'] != df_games['games'].shift()).cumsum()
            df_games['new_games'] = df_games['games'].where(df_games['cambios'] != 1, '0-0')
            ...
            for _, row in df.iterrows():
                ...
                if match_score != previous_match_score:
                    ...
                    if next_row_index < len(df):
                        ...
                        new_row = {'MatchScore': next_row['MatchScore'], 'GameScore': '0:0'}
            ...
            if len(tiebreak)>0:
                ...
                new_tb_df = pd.DataFrame({'MatchScore': [set_anterior + '6-6'] * len(tiebreak),'GameScore': tiebreak})
                ...
                new_tb_df = new_tb_df.rename(columns={'MatchScore': 'MatchScore', 'GameScore': 'GameScore', 'r': 'Recibe', 's': 'Saca'})
                last_game_score = new_tb_df['GameScore'][len(new_tb_df['GameScore'])-1].split(':')
        ...
        try:
            ...
            pbyp['BreakPoint'] = pbyp['GameScore'].apply(lambda x: 1 if x.endswith(('BP', 'BPSP')) else 0)
            pbyp['MatchPoint'] = pbyp['GameScore'].apply(lambda x: 1 if x.endswith(('MP'))else 0)
            pbyp['GameScore'] = pbyp['GameScore'].str.replace('BP', '').str.replace('SP', '').str.replace('MP', '')
        except:
            pbyp={}
            
        if elements[-1] == 'away':
            try:
                pbyp['MatchScore'] = pbyp['MatchScore'].apply(lambda x: ' '.join([score[::-1] for score in x.split(' ')]))
                pbyp['GameScore'] = pbyp['GameScore'].str.split(':').apply(lambda x: x[1] + ':' + x[0])
            except:
                pbyp={}
                
        try:
            for indice, valor in pbyp['MatchScore'].items():
                if valor[-3:] == '6-6' and pbyp.at[indice,'GameScore'] == '0:0' and pbyp.at[indice,'Recibe'] == '0:0':
                    pbyp.drop(indice,inplace=True)
                elif valor[-3:] == '6-6' and pbyp.at[indice,'GameScore'] != '0:0' and pbyp.at[indice,'Recibe'] == '0:0':
    ...
    d_pbyp_check = 'SI'
    pbyp.to_csv("C:/Users/Paula/Documents/Projects/TennisData/FS_matches/pbyp/" + name_file, index=False, header=None)
...
os.chdir('C:/Users/Paula/Documents/Projects/TennisData/FS_tourneys_by_year/')
...
for filename in csv_files:
    ...
    for index, row in links_df.iterrows():
        year_df = row['tourny_year']
        tourney_name_df = row['tourny_name']
        link_df = row['tourney_link']

        print( 'Comienzo a leer ' + str(year_df) + ' ' + str(tourney_name_df))

        link_df = link_df + 'results/'
        if 'australian-open' in link_df.split('/')[-1] or 'us-open' in link_df.split('/')[-1] or 'french-open' in link_df.split('/')[-1] or 'wimbledon' in link_df.split('/')[-1]:
            more_matches = True
        else:
            more_matches = False
        ...
        for one_match in list_of_matches:
            ...
            d_stat_check = 'NO'
            try:
                d_round_match, d_h_player, d_a_player = scrap_match (one_match)
            ...
            if d_pbyp_check == 'NO' or d_simple_check == 'NO' or d_stat_check == 'NO':
                nueva_fila = [contador_partidos, year_df, tourney_name_df, d_pbyp_check, d_simple_check, d_stat_check, 
