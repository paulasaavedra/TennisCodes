...
def tourney_to_matchs(URL, more_matches):
    ...
    OPTIONS.add_argument(
        "--headless"
    )  # Modo headless para evitar que se abra la ventana
    ...
    if more_matches:
        try:
            trying = 2
            while trying > 0:
                morematches_button = driver.find_element(
...
for filename in [csv_files]:
    ...
    for index, row in links_df.iterrows():
        ...
        if (
            "australian-open" in link_df.split("/")[-3]
            or "us-open" in link_df.split("/")[-3]
            or "french-open" in link_df.split("/")[-3]
            or "wimbledon" in link_df.split("/")[-3]
        ):
            more_matches = True
        else:
            more_matches = False
